...
export default function SidebarFiltriCampania({ tipo, categorie }) {
  const {
    categoria, setCategoria,
    tipologia, setTipologia,
    sottoTipologia, setSottoTipologia,
    data, setVoceSelezionata,
  } = useAppState();
  ...
  const handleSelect = (val) => {
    if (tipo === 'categoria') {
      ...
      // Cerca voci foglia in data.col1
      const prefix = `CAM25_${categoria}.${tipologia}.${val}.`;
      const voci = data.filter(item =>
        item.col0?.startsWith(prefix) &&
        item.col0.split('.').length === 4
      );
